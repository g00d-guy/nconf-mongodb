...
var Mongodb = exports.Mongodb = function(opts) {
    ...
    opts = opts || {};
    this.type = 'Mongodb';
    this.namespace = opts.namespace || 'nconf';
    ...
    if ( opts.mongoose ) {
        if ( opts.mongoose.user && opts.mongoose.pass ) {
            this.auth = {
                username: opts.mongoose.user,
                password: opts.mongoose.pass
            };
        }
        this.host = opts.mongoose.db.serverConfig.host;
        this.port = opts.mongoose.db.serverConfig.port;
        this.db   = opts.mongoose.db.databaseName;
    }
...
Mongodb.prototype.save = function(callback) {
    ...
    Object.keys(this.cache.store).forEach(function(key) {
        queue.push(self.cache.store[key]);
    });
};
